              ...
              <ul className="pack">
                {
                  this.state.packData.map((pack, idx) => {
                    return <label key={idx} htmlFor={this.props.rollAlt + `-` + pack.name} className="pack-option" style={pack.quantity === this.state.selectedPackQuantity ? grayStyle : whiteStyle} >
